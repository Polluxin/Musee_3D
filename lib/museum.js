...
/* TODO createTransitionRoom :
    Create a transition room used to move between dedicated rooms.
 */
...
function buildWalls(w, h, d, doorAtFront = 0, doorAtBack = 0, doorAtLeft = 0, doorAtRight = 0) {
    ...
    const b = new THREE.MeshBasicMaterial({ color : 0x492200 });
    ...
    const leftShape = createRectangleShape(h, cap);
    if (doorAtLeft) leftShape.holes.push(doorPath);
    ...
    const rightShape = createRectangleShape(h, cap);
    if (doorAtRight) rightShape.holes.push(doorPath);
    const right = createWall(leftShape, d);
    ...
    if (doorAtFront) frontShape.holes.push(doorPath);
    ...
    if (doorAtBack) backShape.holes.push(doorPath);
