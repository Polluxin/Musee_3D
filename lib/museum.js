...
export let Simple = {
    ...
    computePresentersPosition : function(n) {
        ...
        let i = 0;
        while (i < n/2){
            this.presentersPositions.push(new Vector3(-this.spaceW/2,pos,0.5));
            pos += this.spaceH;
            i += 1;
        }
        pos = deb;
    ...
    computePaintingPosition : function(n) {
        this.paintingPosition = new Vector3(0, this.getWidth(n)/2, 1/8 * this.getHeight(n));
    }
}
...
*/
export function createRoom(nb_objects, images, protagonist = null, configuration = Simple){
    const room = new THREE.Group();

    // Build walls and roof
    let walls = buildWallsSimple(nb_objects, protagonist);
    room.add(walls);
...
function createPresenter(){
    ...
    const msocle = new THREE.Mesh( socle, presenterMaterial );
    ...
    presenter.position.y = 3;
    presenter.position.z = 0.5;
    return presenter;
}
...
*/
function buildWallsSimple(n, image, door = 0){
    ...
    const painting = createPainting(image, 2, 2.5);
    painting.position.set(Simple.paintingPosition.x,
        Simple.paintingPosition.y-1, Simple.paintingPosition.z);
    walls.add(painting);
