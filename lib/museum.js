...
function buildWallsSimple(n, door){
    ...
    const left = mesh2.clone();
    left.rotation.y = Math.PI / 2; // To avoid non collision
    left.position.set(-w/2,0,cap/2);
    walls.add(left);

    const deep = new THREE.PlaneGeometry(w, cap, 25,25);
